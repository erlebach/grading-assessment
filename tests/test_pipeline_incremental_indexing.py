"""Comprehensive test suite for grading_pipeline incremental indexing.
...
All tests use tests/tmp_chroma_indexes/ for temporary artifacts.
Global cleanup ensures each test run starts with a clean slate.
Cleanup behavior controlled by tests/test_config.yaml.
...
from grading_pipeline.manifest import get_manifest_summary, load_manifest


def load_test_config() -> dict:
    """Load test configuration from tests/test_config.yaml.
...
def should_cleanup_artifacts() -> bool:
    """Check if artifacts should be cleaned up based on test config.

    Returns:
        True if artifacts should be removed, False otherwise.

    """
    config = load_test_config()
    cleanup_config = config.get("cleanup", {})
    return cleanup_config.get("remove_artifacts", True)
...
def get_test_artifacts_dir() -> Path:
    ...
    Returns:
        Path to test artifacts directory (tests/tmp_chroma_indexes/).

    """
    tests_dir = Path(__file__).parent
    config = load_test_config()
    paths_config = config.get("paths", {})
    artifacts_dir_name = paths_config.get("chroma_indexes", "tmp_chroma_indexes")
    return tests_dir / artifacts_dir_name


def cleanup_all_test_artifacts() -> Path:
    """Remove all test artifacts and create fresh directory.

    Respects test_config.yaml setting for cleanup behavior.
    Also cleans up old test_tmp directory in grading_pipeline/ if it exists.

    Returns:
        Path to the cleaned test artifacts directory.
    ...
    if old_test_dir.exists():
        print(f"⚠ Found old test directory: {old_test_dir}")
...
def test_fresh_build():
    ...
    test_artifacts_dir = get_test_artifacts_dir()
    persist_dir = test_artifacts_dir / "fresh_build"
...
def test_no_changes():
    ...
    config_path = Path("grading_pipeline/config/sources.yaml")
    test_artifacts_dir = get_test_artifacts_dir()
    persist_dir = test_artifacts_dir / "no_changes"
...
def test_add_new_source():
    ...
    test_artifacts_dir = get_test_artifacts_dir()
    config_path = test_artifacts_dir / "add_source_config.yaml"
    persist_dir = test_artifacts_dir / "add_source"
    ...
    initial_config = {
        "sources": [
            {
                "type": "file",
                "path": str(Path("grading_pipeline/sources").absolute()),
    ...
    source_dir = Path("grading_pipeline/sources")
...
def test_changed_source():
    ...
    test_artifacts_dir = get_test_artifacts_dir()
    test_dir = test_artifacts_dir / "changed_source"
...
def main():
    ...
    print("=" * 70)
    print("Grading Pipeline: Incremental Indexing Test Suite")
    print("=" * 70)
    ...
    try:
        ...
        # Final cleanup
        if should_cleanup_artifacts():
            print("\n[Final Cleanup] Removing all test artifacts...")
            test_artifacts_dir = get_test_artifacts_dir()
            if test_artifacts_dir.exists():
                shutil.rmtree(test_artifacts_dir)
                print(f"✓ Cleaned up: {test_artifacts_dir}")
        else:
            test_artifacts_dir = get_test_artifacts_dir()
        ...
        if should_cleanup_artifacts():
            test_artifacts_dir = get_test_artifacts_dir()
            if test_artifacts_dir.exists():
                print(f"\n[Error Cleanup] Removing test artifacts...")
                shutil.rmtree(test_artifacts_dir)
